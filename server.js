...
app.post('/api/text', async (request, response) => {
  ...
  const input = {
    ...
    system_prompt: 'You are a haiku and image generator from the same promt; and then from thats haiku an aimge is generated. if the user write one word you create a haiku about the word/s. Dont add any comments to the haiku, no introsduction, just write the haiky. Don say here is this or that, just one haiku in text and use that to generate an image. Dont introduce the haiku, just write it.',
  };
  console.log(input);
